...
import { createRerollBridge, createShellRuntimeBindings } from './app/createRuntimeBridges.js';
...
function getDirectorStartIndex() {
    const queue = Array.isArray(AppState.memory?.queue) ? AppState.memory.queue : [];
    const firstPending = queue.findIndex((memory) => {
        const status = String(memory?.chapterOutlineStatus || '').trim().toLowerCase();
        return status !== 'done' && status !== 'failed';
    });
    if (firstPending === -1) return 0;
    return Math.max(0, Math.min(firstPending, Math.max(0, queue.length - 1)));
}

function validateDirectorStartPreconditions() {
    if (AppState.processing.isRunning) {
        ErrorHandler.showUserError('当前已有处理任务运行中，请先停止后再启动新的导演流程');
        return false;
    }
...
handleStartConversion = async () => {
    ErrorHandler.showUserError('世界书提取与去重功能已移除，请使用“仅导演切拍”');
};

handleStartDirectorConversion = async () => {
    saveCurrentSettings();
    if (!validateDirectorStartPreconditions()) return;

    AppState.memory.startIndex = getDirectorStartIndex();
